...
// ... Your mpesawithdrawal code ...
const mpesaatmwithdrawaltrariff = [
    { min: 0, max: 199, new: "insufficient" },
    { min: 200, max: 2500, new: "35" },
...
// ... Your existing code ...
// Function to find M-PESA withdrawal tariff based on the amount
function  findmpesaatmwithdrawaltrariff(amount) {
    for (const tariff of mpesaatmwithdrawaltrariff) {
        if (amount >= tariff.min && amount <= tariff.max) {
            return tariff;
        }
    }
...
function calculate() {
    ...
    // Input for M-PESA ATM withdrawal amount
    const atmWithdrawalAmountInput = document.getElementById("mpesa_amount");
    const atmWithdrawalAmount = parseFloat(atmWithdrawalAmountInput.value);

    // Find the M-PESA ATM withdrawal tariff for the given amount
    const atmWithdrawalTariff = findmpesaatmwithdrawaltrariff(atmWithdrawalAmount);
    ...
    if (mpesaTariff) {
        ...
        document.getElementById('displayATMWithdrawalTariff').textContent = atmWithdrawalTariff.new;
        ...
        document.getElementById('displayATMWithdrawalTariff').textContent = "N/A";
    }
